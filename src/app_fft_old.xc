...
void generate_tworeal_test_signal(int N, int test) {     // in complex signal there are two fields (one for real part and one for imaginary)
    ...
    case 2: {
        printf("++++ Test %d: %d point FFT of two real signals:: re0: %d Hz sine, re1: %d Hz cosine\n"
                 ,test,N,INPUT_FREQ,INPUT_FREQ);
        for(int i=0; i<N; i++) {
            two_re[i].re = lib_dsp_math_multiply(Q31(0.75), sin8(i), 31);
            two_re[i].im = lib_dsp_math_multiply(Q31(0.75), cos8(i), 31);
            two_im[i].re = 0;
            two_im[i].im = 0;
        }
        break;
    ...
    case 3: {
        printf("++++ Test %d: %d point FFT of two real signals:: re0: %d Hz cosine, re1: %d Hz sine\n"
                 ,test,N,INPUT_FREQ,INPUT_FREQ);
        for(int i=0; i<N; i++) {
            two_re[i].re = lib_dsp_math_multiply(Q31(0.33), cos8(i), 31);
            two_re[i].im = lib_dsp_math_multiply(Q31(0.18), sin8(i), 31);
            two_im[i].re = 0;
            two_im[i].im = 0;
        }
        break;
